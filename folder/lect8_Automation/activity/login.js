...
// //puppeteer.launch({headless:false});
 const id= "nimunifi.iludujit@tovinit.com";
 const pw= "divya181999";
...
let browserOpenPromise = puppeteer.launch({
  headless: false,
  defaultViewport: null, //so that everything is visible in full screen
  args: ["--start-maximized"], //to get fullscreen
  executablePath: "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe", //everything we now be executed in chrome
});
...
browserOpenPromise
  ...
  .then(function () {//typing id in sign-in page
    let idTypePromise = tab.type("#input-1", id);
    return idTypePromise;
  })
  .then(function () {//typing pswrd in sign-in page
    let pwTypePromise = tab.type("#input-2", pw);
    return pwTypePromise;
  })
  .then(function () {//click on login button
...
function waitAndClick(selector) {
  return new Promise(function (scb, fcb) {//making our own promisified function
    let waitPromise = tab.waitForSelector(selector, { visible: true });//this will wait until it find given selector
    waitPromise
      .then(function () {
        let clickPromise = tab.click(selector);//after waiting now it will click
        return clickPromise;
      })
      .then(function () {//this will now go to then part of function which called waitAndClick
        scb();
      })
