...
function checkBatsmanPath(TeamName, BatsmanName){
    let BatsmanPath= `./IPL/${TeamName}/${BatsmanName}.json`;
    return fs.existsSync(BatsmanPath);
}

function createTemFolder(TeamName){
    let teamPath= `./IPL/${TeamName}`;
...
function ProcessDetails(TeamName, BatsmanName, runs, balls, fours, sixes, strikeRate){
    let TeamExists= checkTeamFolder(TeamName);
    if(TeamExists){
        let BatsmanExist= checkBatsmanPath(TeamName, BatsmanName);
        if(BatsmanExist){
